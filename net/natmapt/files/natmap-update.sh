...
. /usr/lib/natmap/status.sh
INITD='/etc/init.d/natmap'
# Section status (private per-PID + public per-name) files, the atomic
# writes, the filename sanitizing and the schema all live in the section
# status store — see /usr/lib/natmap/status.sh header for the contract.
...
# Section status for this mapping event (private + public, one call).
status_publish "$SECTIONID" "$COMMENT" "$STATUS_NAME" "$PPID" \
	"$ip" "$port" "$ip4p" "$inner_port" "$protocol" "$inner_ip"
